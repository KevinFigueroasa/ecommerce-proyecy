...
export const db = {
  items: localStorage.getItem('products') ? JSON.parse(localStorage.getItem('products')) : items,
  methods: {
    find: (id) => {
  ...
  productsButton.forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (product && product.quantity > 0) {
        ...
        alert('No tenemos más en Stock')
      }
    })
  })
  localStorage.setItem('products', JSON.stringify(db.items))
}